...
**Codex is mandatory on everything Becky builds or changes — essential, non-negotiable, no exceptions.** Extended to Becky 2026-08-08 by Kevin's own direct instruction, matching the identical requirement already fixed into `begb0037admin/markey`'s and `begb0037admin/zara`'s `AGENT.md` after a real echo-terminal-tts Mac-port build where an ad-hoc Codex read-only pass caught concrete, verified bugs live testing alone had missed. For any real implementation work (not pure read-only research/reporting), consult Codex via `codex exec -s read-only -o <result-file>` (see `begb0037admin/agent-commons` or the coordinator for the exact invocation pattern) at three points, minimum:
1. **Before starting** — review the plan itself against the relevant brief/spec.
2. **At each meaningful implementation step** — review that step's diff/output before moving to the next.
3. **Once more as a full end-to-end pass** — over the complete change, checking cross-step consistency, before it is shown to Kevin or pushed anywhere.
